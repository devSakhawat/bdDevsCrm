...
public interface ICrmInstituteService
{
	...
	/// </summary>
	/// <param name="entityForCreate">The DTO containing data for the new institute.</param>
	/// <param name="currentUser">The DTO containing current user information.</param>
	/// <param name="cancellationToken">Token to monitor for cancellation requests.</param>
	/// <returns>The created <see cref="CrmInstituteDto"/> with the newly assigned ID.</returns>
	/// <exception cref="BadRequestException">Thrown when <paramref name="entityForCreate"/> is null.</exception>
	/// <exception cref="InvalidCreateOperationException">Thrown when InstituteId is not 0 for new creation.</exception>
	/// <exception cref="DuplicateRecordException">Thrown when an institute with the same name already exists.</exception>
	Task<CrmInstituteDto> CreateInstituteAsync(CrmInstituteDto entityForCreate, UsersDto currentUser, CancellationToken cancellationToken = default);

	/// <summary>
	/// Updates an existing institute record.
	/// </summary>
	/// <param name="instituteId">The ID of the institute to update.</param>
	...
	/// <param name="cancellationToken">Token to monitor for cancellation requests.</param>
	/// <returns>The updated <see cref="CrmInstituteDto"/> reflecting the saved state.</returns>
	...
	Task<CrmInstituteDto> UpdateInstituteAsync(int instituteId, CrmInstituteDto modelDto, bool trackChanges, CancellationToken cancellationToken = default);

	/// <summary>
	/// Deletes an institute record identified by the given ID.
	/// </summary>
	/// <param name="instituteId">The ID of the institute to delete.</param>
	/// <param name="trackChanges">Indicates whether EF change tracking should be enabled.</param>
	/// <param name="cancellationToken">Token to monitor for cancellation requests.</param>
	/// <returns>The number of affected rows.</returns>
	/// <exception cref="IdParametersBadRequestException">Thrown when <paramref name="instituteId"/> is zero or negative.</exception>
	/// <exception cref="NotFoundException">Thrown when no institute record is found for the given ID.</exception>
	Task<int> DeleteInstituteAsync(int instituteId, bool trackChanges, CancellationToken cancellationToken = default);

    ...
    if (filter != null && (filter.Filters != null && filter.Filters.Count > 0))
    {
      ...
      for (var i = 0; i < filters.Count; i++)
      {
        if (i == 0)
        {
          ...
          whereClause += string.Format(" {1}", UtilityCommon.ToLinqOperator(filter.Logic), UtilityCommon.BuildWhereClause<T>(i, filter.Logic,
        ...
        else
        {
          if (filters[i].Value != null)
          {
            whereClause += string.Format(" {0} {1}",  UtilityCommon.ToLinqOperator(filter.Logic), UtilityCommon.BuildWhereClause<T>(i, filter.Logic,
                    filters[i], parameters));
          }
        }
      }

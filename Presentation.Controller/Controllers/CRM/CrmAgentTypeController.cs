...
/// <summary>
/// CrmAgentType management endpoints.
/// </summary>
[AuthorizeUser]
public class CrmAgentTypeController : BaseApiController
{
    private readonly IMemoryCache _cache;

    public CrmAgentTypeController(IServiceManager serviceManager, IMemoryCache cache) : base(serviceManager)
    {
        _cache = cache;
    }
    ...
    /// </summary>
    [HttpPost(RouteConstants.CrmAgentTypeSummary)]
    public async Task<IActionResult> SummaryAsync([FromBody] GridOptions options, CancellationToken cancellationToken = default)
    {
        if (options == null)
            throw new NullModelBadRequestException(nameof(GridOptions));

        var summaryGrid = await _serviceManager.CrmAgentTypes.AgentTypesSummaryAsync(options, cancellationToken);

        if (!summaryGrid.Items.Any())
            return Ok(ApiResponseHelper.Success(new GridEntity<CrmAgentTypeDto>(), "No records found."));
    ...
    /// </summary>
    [HttpPost(RouteConstants.CreateCrmAgentType)]
    ...
    /// </summary>
    [HttpPut(RouteConstants.UpdateCrmAgentType)]
    [ServiceFilter(typeof(EmptyObjectFilterAttribute))]
    public async Task<IActionResult> UpdateAsync([FromRoute] int key, [FromBody] UpdateCrmAgentTypeRecord record, CancellationToken cancellationToken = default)
    {
    ...
    /// </summary>
    [HttpDelete(RouteConstants.DeleteCrmAgentType)]
    ...
    /// </summary>
    [HttpGet(RouteConstants.ReadCrmAgentType)]
    public async Task<IActionResult> GetByIdAsync([FromRoute] int id, CancellationToken cancellationToken = default)
    {
        if (id <= 0)
            throw new IdParametersBadRequestException();

        var record = await _serviceManager.CrmAgentTypes.AgentTypeAsync(id, trackChanges: false, cancellationToken: cancellationToken);
    ...
    /// </summary>
    [HttpGet(RouteConstants.ReadCrmAgentTypes)]
    public async Task<IActionResult> GetAllAsync(CancellationToken cancellationToken = default)
    {
        var records = await _serviceManager.CrmAgentTypes.AgentTypesAsync(trackChanges: false, cancellationToken: cancellationToken);

        if (!records.Any())
            return Ok(ApiResponseHelper.Success(Enumerable.Empty<CrmAgentTypeDto>(), "No records found."));
